...
#include <stm32f1xx.h>
    ...
    static inline gpio_pin_t STM32_PIN(GPIO_TypeDef *GPIOx, uint32_t pin)
    {
        ...
        (RCC->APB2ENR |= (1UL << (2UL + (((uint32_t)(GPIOx)-GPIOA_BASE) / (GPIOB_BASE - GPIOA_BASE)))));
...
#define GET_TICK(TICK) (TICK = DWT->CYCCNT)

/** @brief Convert microseconds to ticks
 *  @param US microseconds value
 *  @return ticks value
 */
#define US_TO_TICKS(US) ((uint32_t)(US) * (SystemCoreClock / 1000000UL))
