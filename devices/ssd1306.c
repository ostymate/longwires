/**
 * @file ssd1306.c
 * @brief SSD1306 OLED display driver for teletype and page text mode
 */

#include "ssd1306.h"
#include <stdarg.h>

#define SSD1306_I2C_ADDR_DEFAULT 0x3C
#define SSD1306_I2C_ADDR_ALT 0x3D

#define CMD_BYTE 0x00
#define DATA_BYTE 0x40

#define CMD_SET_PAGE_START 0xB0
#define CMD_SET_COL_LOW 0x00
#define CMD_SET_COL_HIGH 0x10
#define CMD_SET_DISP_OFF 0xAE
#define CMD_SET_DISP_ON 0xAF
#define CMD_SET_CONTRAST 0x81
#define CMD_SEG_REMAP 0xA0
#define CMD_SEG_REMAP_FLIP 0xA1
#define CMD_COM_SCAN_NORMAL 0xC0
#define CMD_COM_SCAN_FLIP 0xC8
#define CMD_SET_MULTIPLEX 0xA8
#define CMD_SET_DISP_OFFSET 0xD3
#define CMD_SET_CLOCK_DIV 0xD5
#define CMD_SET_PRECHARGE 0xD9
#define CMD_SET_COM_PINS 0xDA
#define CMD_SET_VCOM_DESEL 0xDB
#define CMD_ENTIRE_DISP_ON 0xA4
#define CMD_NORMAL_DISP 0xA6
#define CMD_CHARGE_PUMP 0x8D
#define CMD_CHARGE_PUMP_ENABLE 0x14

#define PAGE_HEIGHT 8
#define PAGE_COUNT (OLED_HEIGHT / PAGE_HEIGHT)
#define CHAR_WIDTH 5
#define CHAR_PER_PAGE (OLED_WIDTH / CHAR_WIDTH)

#define FIRST_CHAR 0x20
#define LAST_CHAR 0x7E

static const uint8_t font[95][5] = {
    {0x00,0x00,0x00,0x00,0x00}, /* space */
    {0x00,0x00,0x5F,0x00,0x00}, /* ! */
    {0x00,0x07,0x00,0x07,0x00}, /* " */
    {0x14,0x7F,0x14,0x7F,0x14}, /* # */
    {0x24,0x2A,0x7F,0x2A,0x12}, /* $ */
    {0x23,0x13,0x08,0x64,0x62}, /* % */
    {0x36,0x49,0x49,0x26,0x50}, /* & */
    {0x00,0x00,0x03,0x03,0x00}, /* ' */
    {0x00,0x1C,0x22,0x41,0x00}, /* ( */
    {0x00,0x41,0x22,0x1C,0x00}, /* ) */
    {0x14,0x08,0x3E,0x08,0x14}, /* * */
    {0x08,0x08,0x3E,0x08,0x08}, /* + */
    {0x00,0x50,0x30,0x00,0x00}, /* , */
    {0x08,0x08,0x08,0x08,0x08}, /* - */
    {0x00,0x60,0x60,0x00,0x00}, /* . */
    {0x20,0x10,0x08,0x04,0x02}, /* / */
    {0x3E,0x51,0x49,0x45,0x3E}, /* 0 */
    {0x00,0x42,0x7F,0x40,0x00}, /* 1 */
    {0x42,0x61,0x51,0x49,0x46}, /* 2 */
    {0x21,0x41,0x45,0x4B,0x31}, /* 3 */
    {0x18,0x14,0x12,0x7F,0x10}, /* 4 */
    {0x27,0x45,0x45,0x45,0x39}, /* 5 */
    {0x3C,0x4A,0x49,0x49,0x30}, /* 6 */
    {0x01,0x71,0x09,0x05,0x03}, /* 7 */
    {0x36,0x49,0x49,0x49,0x36}, /* 8 */
    {0x06,0x49,0x49,0x29,0x1E}, /* 9 */
    {0x00,0x36,0x36,0x00,0x00}, /* : */
    {0x00,0x56,0x36,0x00,0x00}, /* ; */
    {0x08,0x14,0x22,0x41,0x00}, /* < */
    {0x14,0x14,0x14,0x14,0x14}, /* = */
    {0x00,0x41,0x22,0x14,0x08}, /* > */
    {0x02,0x01,0x51,0x09,0x06}, /* ? */
    {0x7F,0x41,0x5D,0x5D,0x4F}, /* @ */
    {0x7E,0x11,0x11,0x11,0x7E}, /* A */
    {0x7F,0x49,0x49,0x49,0x36}, /* B */
    {0x3E,0x41,0x41,0x41,0x22}, /* C */
    {0x7F,0x41,0x41,0x22,0x1C}, /* D */
    {0x7F,0x49,0x49,0x49,0x41}, /* E */
    {0x7F,0x09,0x09,0x09,0x01}, /* F */
    {0x3E,0x41,0x49,0x49,0x7A}, /* G */
    {0x7F,0x08,0x08,0x08,0x7F}, /* H */
    {0x00,0x41,0x7F,0x41,0x00}, /* I */
    {0x20,0x40,0x41,0x3F,0x01}, /* J */
    {0x7F,0x08,0x14,0x22,0x41}, /* K */
    {0x7F,0x40,0x40,0x40,0x40}, /* L */
    {0x7F,0x02,0x0C,0x02,0x7F}, /* M */
    {0x7F,0x04,0x08,0x10,0x7F}, /* N */
    {0x3E,0x41,0x41,0x41,0x3E}, /* O */
    {0x7F,0x09,0x09,0x09,0x06}, /* P */
    {0x3E,0x41,0x51,0x21,0x5E}, /* Q */
    {0x7F,0x09,0x19,0x29,0x46}, /* R */
    {0x46,0x49,0x49,0x49,0x31}, /* S */
    {0x01,0x01,0x7F,0x01,0x01}, /* T */
    {0x3F,0x40,0x40,0x40,0x3F}, /* U */
    {0x1F,0x20,0x40,0x20,0x1F}, /* V */
    {0x3F,0x40,0x38,0x40,0x3F}, /* W */
    {0x63,0x14,0x08,0x14,0x63}, /* X */
    {0x07,0x08,0x70,0x08,0x07}, /* Y */
    {0x61,0x51,0x49,0x45,0x43}, /* Z */
    {0x00,0x7F,0x41,0x41,0x00}, /* [ */
    {0x02,0x04,0x08,0x10,0x20}, /* \ */
    {0x00,0x41,0x41,0x7F,0x00}, /* ] */
    {0x04,0x02,0x01,0x02,0x04}, /* ^ */
    {0x40,0x40,0x40,0x40,0x40}, /* _ */
    {0x00,0x01,0x02,0x04,0x00}, /* ` */
    {0x70,0x28,0x24,0x28,0x70}, /* a */
    {0x00,0x7E,0x48,0x48,0x30}, /* b */
    {0x38,0x44,0x44,0x44,0x20}, /* c */
    {0x00,0x30,0x48,0x48,0x7F}, /* d */
    {0x38,0x54,0x54,0x44,0x00}, /* e */
    {0x00,0x7C,0x14,0x14,0x04}, /* f */
    {0x38,0x44,0x54,0x74,0x00}, /* g */
    {0x7F,0x08,0x04,0x04,0x78}, /* h */
    {0x00,0x00,0x7A,0x00,0x00}, /* i */
    {0x20,0x40,0x44,0x3D,0x00}, /* j */
    {0x7F,0x10,0x28,0x44,0x00}, /* k */
    {0x00,0x02,0x7E,0x40,0x00}, /* l */
    {0x7C,0x04,0x78,0x04,0x78}, /* m */
    {0x7C,0x08,0x04,0x04,0x78}, /* n */
    {0x38,0x44,0x44,0x44,0x38}, /* o */
    {0x7E,0x12,0x12,0x0C,0x00}, /* p */
    {0x00,0x0C,0x12,0x12,0x7E}, /* q */
    {0x7C,0x08,0x04,0x04,0x08}, /* r */
    {0x58,0x54,0x54,0x54,0x34}, /* s */
    {0x00,0x08,0x7C,0x48,0x00}, /* t */
    {0x3C,0x40,0x40,0x20,0x7C}, /* u */
    {0x1C,0x20,0x40,0x20,0x1C}, /* v */
    {0x3C,0x40,0x30,0x40,0x3C}, /* w */
    {0x44,0x28,0x10,0x28,0x44}, /* x */
    {0x00,0x5C,0x50,0x50,0x7C}, /* y */
    {0x44,0x64,0x54,0x4C,0x44}, /* z */
    {0x00,0x08,0x36,0x41,0x00}, /* { */
    {0x00,0x00,0x7F,0x00,0x00}, /* | */
    {0x00,0x41,0x36,0x08,0x00}, /* } */
    {0x08,0x04,0x08,0x10,0x08}, /* ~ */
};

bool oled_print_page(oled_t *dev, const char *str, uint8_t page)
{
    uint8_t *buf = dev->buf;

    /* Scroll and set position */
    buf[0] = CMD_BYTE;
    buf[1] = CMD_SET_DISP_OFFSET;
    buf[2] = dev->scroll_offset;
    buf[3] = CMD_BYTE;
    buf[4] = CMD_SET_PAGE_START | page;
    buf[5] = CMD_BYTE;
    buf[6] = CMD_SET_COL_LOW;
    buf[7] = CMD_BYTE;
    buf[8] = CMD_SET_COL_HIGH;

    if (!i2c_write(&dev->i2c_device, buf, 9, true))
        return false; /* Failed to set position */

    /* Fill data buffer */
    buf[0] = DATA_BYTE;
    uint8_t pos = 1; /* Start of page data */
    for (uint8_t i = 0; i < CHAR_PER_PAGE; i++)
    {
        if ((str[i] < FIRST_CHAR || str[i] > LAST_CHAR))
            break; /* End of string or unsupported character */

        for (uint8_t j = 0; j < CHAR_WIDTH; j++)
            buf[pos++] = font[str[i] - FIRST_CHAR][j];
    }

    /* clear end of the line */
    while (pos < BUF_SIZE)
        buf[pos++] = 0;

    if (!i2c_write(&dev->i2c_device, buf, BUF_SIZE, true))
        return false; /* Failed to write data */

    return true; 
}

bool oled_init(oled_t *dev, gpio_pin_t sda_pin, gpio_pin_t scl_pin, bool use_default_addr, bool flip)
{
    i2c_init(&dev->i2c_device, sda_pin, scl_pin,
             use_default_addr ? SSD1306_I2C_ADDR_DEFAULT : SSD1306_I2C_ADDR_ALT);

    uint8_t *buf = dev->buf;
    buf[0] = CMD_BYTE;
    buf[1] = CMD_SET_DISP_OFF;
    buf[2] = CMD_SET_CLOCK_DIV;
    buf[3] = 0x80;
    buf[4] = CMD_SET_MULTIPLEX;
    buf[5] = 0x3F;
    buf[6] = CMD_CHARGE_PUMP;
    buf[7] = CMD_CHARGE_PUMP_ENABLE;
    buf[8] = flip ? CMD_SEG_REMAP_FLIP : CMD_SEG_REMAP;
    buf[9] = flip ? CMD_COM_SCAN_FLIP : CMD_COM_SCAN_NORMAL;
    buf[10] = CMD_SET_COM_PINS;
    buf[11] = 0x12;
    buf[12] = CMD_SET_CONTRAST;
    buf[13] = 0x7F;
    buf[14] = CMD_SET_PRECHARGE;
    buf[15] = 0xF1;
    buf[16] = CMD_SET_VCOM_DESEL;
    buf[17] = 0x40;
    buf[18] = CMD_ENTIRE_DISP_ON;
    buf[19] = CMD_NORMAL_DISP;
    buf[20] = CMD_SET_DISP_ON;

    if (!i2c_write(&dev->i2c_device, buf, 21, true))
        return false; /* Failed to initialize display */

    dev->scroll_offset = 0;
    /* Clear display */
    for (uint8_t page = 0; page < PAGE_COUNT; page++)
    {
        if (!oled_print_page(dev, "", page))
            return false; /* Failed to clear display */
    }

    return true;
}

void oled_print(oled_t *dev, const char *str)
{
    while (*str) {
        dev->scroll_offset += PAGE_HEIGHT;
        oled_print_page(dev, str, ((dev->scroll_offset / PAGE_HEIGHT) + (PAGE_COUNT - 1)) % PAGE_COUNT);
        
        /* Skip exactly CHAR_PER_PAGE printed chars */
        uint8_t skipped = 0;
        while (*str && *str != '\n' && skipped < CHAR_PER_PAGE) {
            str++;
            skipped++;
        }
        if (*str == '\n') str++;
    }
}